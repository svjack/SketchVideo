...
class DDIMSchedulerInversionOutput(BaseOutput):
    """
    Output class for the scheduler's `step` function output.
...
class CogVideoXDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def inversion_step(
        ...
        # 2. compute alphas, betas
        alpha_prod_t = self.alphas_cumprod[timestep] if timestep >= 0 else self.alphas_cumprod[0] # self.final_alpha_cumprod
        ...
        a_t = ((1 - alpha_prod_t_next) / (1 - alpha_prod_t)) ** 0.5
        b_t = alpha_prod_t_next**0.5 - alpha_prod_t**0.5 * a_t

        next_sample = a_t * sample + b_t * pred_original_sample

        if not return_dict:
            return (next_sample,)
        
        return DDIMSchedulerInversionOutput(next_sample=next_sample, pred_original_sample=pred_original_sample)
