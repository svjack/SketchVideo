...
class CogVideoXInvertControlNetPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        sketch_mask_1_16 = F.interpolate(sketch_mask, scale_factor=0.0625)
        # DownSample the mask into latent space
        video_mask = 1.0 - sketch_mask_1_16
        # The first frame is processed separately
        sketch_mask_latent = [sketch_mask_1_16[0]]
        ...
        for i in range(1, video_mask.shape[0]):
            ...
            sketch_mask_union = torch.max(sketch_mask_union, sketch_mask_1_16[i])
        ...
        # =============================================================================
        # latent fusion mask
        sketch_mask_1_8 = F.interpolate(sketch_mask, scale_factor=0.125)
        sketch_mask_latent = [sketch_mask_1_8[0]]
        # prepare the masks union buf
        sketch_mask_latent_union = torch.zeros(sketch_mask_latent[0].shape).to(latents)
        for i in range(1, sketch_mask.shape[0]):
            # For f1,f2,f3,f4, union the mask in latent space
            sketch_mask_latent_union = torch.max(sketch_mask_latent_union, sketch_mask_1_8[i])
            if i % 4 == 0:
                # for f4, append union mask into list, clear the union buf
                sketch_mask_latent.append(sketch_mask_latent_union)
                sketch_mask_latent_union = torch.zeros(sketch_mask_latent[0].shape).to(latents)
        sketch_mask_latent = torch.stack(sketch_mask_latent).unsqueeze(0) # [B, T, 1, H, W]
        # ===============================================================================
        ...
        # [999, 979, ..., 39, 19] -> [979, 959, ..., 19, -1]
        timestep_invertion = timesteps - self.scheduler.config.num_train_timesteps // self.scheduler.num_inference_steps
        # [979, 959, ..., 19, -1] -> [-1, 19, ..., 959, 979]
        timestep_invertion = torch.flip(timestep_invertion, dims=[0])
        
        video_input_latent = self.vae.encode(video_input).latent_dist.sample() * self.vae.config.scaling_factor
        # [B, C, T, H, W] -> [B, T, C, H, W]
        video_input_latent = video_input_latent.permute(0,2,1,3,4)

        inversion_latent_list = [video_input_latent]
        
        print("video_input_latent", video_input_latent.shape)
        print("sketch_mask_latent", sketch_mask_latent.shape)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timestep_invertion):
                if self.interrupt:
                    continue

                latent_model_input = torch.cat([video_input_latent] * 2) if do_classifier_free_guidance else video_input_latent
                ...
                if not isinstance(self.scheduler, CogVideoXDPMScheduler):
                    video_input_latent = self.scheduler.inversion_step(noise_pred, t, video_input_latent, **extra_step_kwargs, return_dict=False)[0]
                else:
                    video_input_latent, old_pred_original_sample = self.scheduler.inversion_step(
                        noise_pred,
                        old_pred_original_sample,
                        t,
                        timestep_invertion[i - 1] if i > 0 else None,
                        video_input_latent,
                        **extra_step_kwargs,
                        return_dict=False,
                    )
                video_input_latent = video_input_latent.to(prompt_embeds.dtype)
                
                if i < num_inference_steps - 1:
                    inversion_latent_list.append(video_input_latent)
                ...
                if callback_on_step_end is not None:
                    ...
                    video_input_latent = callback_outputs.pop("latents", video_input_latent)
                    prompt_embeds = callback_outputs.pop("prompt_embeds", prompt_embeds)
                    negative_prompt_embeds = callback_outputs.pop("negative_prompt_embeds", negative_prompt_embeds)

                if i == len(timestep_invertion) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if i == 25:
                    original_video_latent = inversion_latent_list[num_inference_steps - 1 - i]
                    latents = latents * sketch_mask_latent + original_video_latent * (1 - sketch_mask_latent)
