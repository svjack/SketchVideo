...
def get_embedding_frame(
    ...
    if not self.config.use_rotary_positional_embeddings:
        ...
        image_pos_embeds = rearrange(torch.stack(image_pos_embeds_list), 'B T L C -> B (T L) C')
...
def forward(
    ...
    for i, block in enumerate(self.transformer_blocks):
        if i in control_block_index:
            residual_features, sketch_hidden_states = controlNet(
    ...
    if not self.config.use_rotary_positional_embeddings:
        ...
        hidden_states = self.norm_final(hidden_states)[:, text_seq_length:]
